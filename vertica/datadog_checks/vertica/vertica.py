...
class VerticaCheck(AgentCheck):
    ...
    def query_host_resources(self):
        ...
        for host in self.iter_rows(views.HostResources):
            ...
            self.gauge('swap.total', total, tags=tags)
            self.gauge('swap.usable', usable, tags=tags)
            self.gauge('swap.used', used, tags=tags)

            if total:
                utilized = used / total * 100
            else:
                utilized = 0
            self.gauge('swap.utilized', utilized, tags=tags)
